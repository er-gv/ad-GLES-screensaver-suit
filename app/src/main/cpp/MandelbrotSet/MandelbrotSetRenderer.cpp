...
#include "MandelbrotSetRenderer.h"
#include <jni.h>
#include "utils/Logger.h"
#include "graphics/GLUtils.h"
//#include "utils/Scene.h"
#include <cmath>
#define  JULIA_TAG "MandelbrotSetRenderer"
...
bool MandelbrotSetRenderer::compileShaders(){
    // Set program handles
    const char* vertex_shader = GLUtils::openTextFile("shaders/mandelbrot/vertex.glsl");
    const char* fragment_shader = GLUtils::openTextFile("shaders/mandelbrot/fragment.glsl");
    ...
    vertex_shader = GLUtils::openTextFile("shaders/lightSource/pointLight_vertex.glsl");
    fragment_shader = GLUtils::openTextFile("shaders/lightSource/pointlight_fragment.glsl");
...
void MandelbrotSetRenderer::init(){
    if(compileShaders())
        create();
}

void MandelbrotSetRenderer::create(){
...
void MandelbrotSetRenderer::surfaceChange(int width, int height){
    LOG_D(JULIA_TAG, "@MandelbrotSetRenderer::nativeSurfaceChange");
...
void MandelbrotSetRenderer::drawScene(){
...
GLfloat* MandelbrotSetRenderer::getSpectrumParams() {
// R, G, B, A
...
bool MandelbrotSetRenderer::setupBuffers() {
    glGenBuffers(1, &vbo);
    glGenBuffers(1, &ibo);
    if(0== vbo || 0 == ibo){
        return false;
...
void MandelbrotSetRenderer::initAttribsAndUniforms() {
 ...
 * Draws a cube.
 */
void MandelbrotSetRenderer::drawCube(float radians, int idx){
 ...
 */
void MandelbrotSetRenderer::drawLight(){
...
GLfloat MandelbrotSetRenderer::tick() {
...
void MandelbrotSetRenderer::randomizeSpectrumIndices() {
    for(int i=0; i<5; ++i){
        perm[i] =i;
    }
...
static MandelbrotSetRenderer *renderer;

extern "C" JNIEXPORT void JNICALL
Java_com_ergv_glScreenSavers_MandelbrotSet_MandelbrotSetRenderer_nativeSurfaceCreate(
...
extern "C" JNIEXPORT void JNICALL
Java_com_ergv_glScreenSavers_MandelbrotSet_MandelbrotSetRenderer_nativeSurfaceChange(
...
extern "C" JNIEXPORT void JNICALL
Java_com_ergv_glScreenSavers_MandelbrotSet_MandelbrotSetRenderer_nativeDrawFrame(JNIEnv *env, jclass type) {
